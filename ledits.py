...
class RFInversionFluxPipelineSega(
    DiffusionPipeline,
    FluxLoraLoaderMixin,
    FromSingleFileMixin,
    ...
    def encode_prompt(
            ...
            pooled_editing_prompt: Optional[torch.FloatTensor] = None,
    ):
        r"""
        ...
        editing_prompt = [editing_prompt] if isinstance(editing_prompt, str) else editing_prompt
        if editing_prompt_embeds is None and editing_prompt is not None:
                # We only use the pooled prompt output from the CLIPTextModel
                pooled_editing_prompt = self._get_clip_prompt_embeds(
                    prompt=editing_prompt,
                    device=device,
                    num_images_per_prompt=num_images_per_prompt,
                )
                editing_prompt_embeds = self._get_t5_prompt_embeds(
                    prompt=editing_prompt,
                    num_images_per_prompt=num_images_per_prompt,
                    max_sequence_length=max_sequence_length,
                    device=device,
    ...
    def __call__(
        ...
        if enable_edit_guidance and fake_cfg:
            ...
            pooled_prompt_embeds = torch.cat([pooled_prompt_embeds, pooled_neg_prompt_embeds, pooled_editing_prompt],
                                             dim=0)
        ...
        latent_image_ids = torch.cat([latent_image_ids] * (
                1 + enabled_editing_prompts)) if self.do_classifier_free_guidance else latent_image_ids
        text_ids = torch.cat(
            [text_ids] * (1 + enabled_editing_prompts)) if self.do_classifier_free_guidance else text_ids

...
class FluxSemanticImageGuidancePipeline(
    DiffusionPipeline,
    FluxLoraLoaderMixin,
    FromSingleFileMixin,
    ...
    def __call__(
        ...
        max_sequence_length: int = 512,
        sega_ip_adapter_image: Optional[PipelineImageInput] = None,
        sega_ip_adapter_image_embeds: Optional[List[torch.Tensor]] = None,
        sega_ip_adapter_scale: Optional[float] = None,
        ...
        if sega_ip_adapter_image:
            enable_edit_guidance = True
            enabled_editing_prompts = 1
        elif sega_ip_adapter_image_embeds is not None:
            enable_edit_guidance = True
            enabled_editing_prompts = 1
        else:
        ...
        (image_prompt_embeds, image_pooled_prompt_embeds, image_text_ids) = self.encode_prompt(
            prompt="An image",
            prompt_2="An image",
            pooled_prompt_embeds=None,
            lora_scale=lora_scale,
        ...
        sega_ip_adapter_image_embeds = None
        if sega_ip_adapter_image is not None or sega_ip_adapter_image_embeds is not None:
            sega_ip_adapter_image_embeds = self.prepare_ip_adapter_image_embeds(
                sega_ip_adapter_image,
                sega_ip_adapter_image_embeds,
                device,
                batch_size * num_images_per_prompt,
            )
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                if self.interrupt:
                    continue
                if sega_ip_adapter_image_embeds is not None:
                    self._joint_attention_kwargs["ip_adapter_image_embeds"] = sega_ip_adapter_image_embeds
                # if image_embeds is not None:
                #     self._joint_attention_kwargs["ip_adapter_image_embeds"] = image_embeds
                # else: # pop
                #     self._joint_attention_kwargs.pop("ip_adapter_image_embeds", None)
                ...
                if use_sega and enable_edit_guidance and max_edit_cooldown_steps >= i >= min_edit_warmup_steps:
                    # if sega_ip_adapter_image_embeds is not None:
                    #     self._joint_attention_kwargs["ip_adapter_image_embeds"] = sega_ip_adapter_image_embeds
                    # else:  # pop
                    #     self._joint_attention_kwargs.pop("ip_adapter_image_embeds", None)
                    ...
                    noise_pred_sega = self.transformer(
                        hidden_states=latents,
                        timestep=timestep / 1000,
                        guidance=guidance,
                        pooled_projections=image_pooled_prompt_embeds,
                        encoder_hidden_states=image_prompt_embeds,
                        txt_ids=image_text_ids,
                        img_ids=latent_image_ids,
                        joint_attention_kwargs=self.joint_attention_kwargs,
                        return_dict=False,
                    )[0]
                    noise_pred_edit_concepts.append(noise_pred_sega)
                ...
                if use_sega and enable_edit_guidance and max_edit_cooldown_steps >= i >= min_edit_warmup_steps:
                    ...
                    for c, noise_pred_edit_concept in enumerate(noise_pred_edit_concepts):
                        if isinstance(sega_ip_adapter_scale, list):
                            edit_guidance_scale_c = sega_ip_adapter_scale[c]
                        else:
                            edit_guidance_scale_c = sega_ip_adapter_scale
